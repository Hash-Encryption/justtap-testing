...
| ----------------------------------- | ------------------------------------------ | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Hard-coded admin credentials        | Fully resolved in Phase 2                  | Custom admin route deleted; environment-token login retired. Administration requires Supabase `user_roles.role = 'admin'`. | Complete. Rotate historical credentials if used externally.                                              |
...
| Client-controlled `plan_tier`       | Fully resolved                             | Clients have zero write access to `plan_tier`. Modifications require audited `admin_set_entitlement` or server `start_pro_trial()`. | Complete.                                                                                               |
...
| Custom admin authentication         | Fully resolved in Phase 2                  | `/api/admin-auth.ts` deleted. Admin operations run through audited RPCs with `require_admin()` RLS protection. | Complete.                                                                                               |
