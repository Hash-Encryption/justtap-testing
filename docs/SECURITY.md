...
| Broad anonymous lead insertion      | Fixed in Connections Upgrade Phase 01      | Direct table inserts are revoked; `create_public_connection` validates bounded public fields and resolves only active cards by slug.                          | Add durable abuse controls and retention only when the later product phase defines them.                |
| Broad anonymous analytics insertion | Fixed in Connections Upgrade Phase 02      | Direct table inserts are revoked; `record_public_card_event` validates canonical events and metadata, derives active cards by slug, and deduplicates retries. | Add durable abuse controls only when later measured/product requirements justify them.                  |
