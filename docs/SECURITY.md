...
| Public/private card boundary        | Partially mitigated                        | Public route/vCard/OG use a server-mapped narrow browser model, but a live anonymous Phase 01 query could still select an inactive row directly from `cards`. | Complete schema/RLS/API separation in Phase 02.                                               |
